...
def update_annotations(filename,image_size,label_path,save_path):
    ...
    data['x_center_ratio'] = np.clip(data['x_center_ratio'], 0, 1)
    data['y_center_ratio'] = data['y_center'].astype(float) / image_size
    data['y_center_ratio'] = np.clip(data['y_center_ratio'], 0, 1)
    ...
    data['width_ratio'] = np.clip(data['width_ratio'], 0, 1)
    ...
    data['height_ratio'] = np.clip(data['height_ratio'], 0, 1)
...
def makelabels():
    label_path = 'E:/deeplearning/dataset/VEDI/1024/raw/Annotations1024/'
    save_path = 'E:/deeplearning/dataset/VEDI/1024/raw/labels/'
