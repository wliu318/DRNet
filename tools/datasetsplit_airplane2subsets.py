...
source_path = 'E:/deeplearning/dataset/simd/yolo/test_only_airplane'  # 原始的标签路径

save_file_path = 'E:/deeplearning/dataset/simd/yolo/test_small_airplane'  # 修改后的标签路径
...
for name in tqdm(labels_name):   # 遍历每一个文件
    ...
    if is_txt_file(txtname):
        ...
        for j in fline:  # 遍历txt文件中每一行
            data_line = j.split()
            if float(data_line[3]) * width * float(data_line[4]) * height < plane_maxsize :
