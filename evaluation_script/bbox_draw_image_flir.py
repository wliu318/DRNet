...
def calculate_iou(box1, box2):
    ...
    计算两个边界框的IoU
    参数:
    box1, box2 - 边界框，格式为(xmin, ymin, xmax, ymax)
    返回值:
    IoU值，介于0和1之间
    """
    # 计算两个边界框的交集部分
...
def plot_one_box(tp_boxes, fn_boxes, fp_boxes, im, line_thickness=2,coloridx=1):
    ...
    for box in tp_boxes:
        color = (0, 255, 0)   #colors(coloridx)   green
        c1, c2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
        cv2.rectangle(im, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)

    for box in fn_boxes:
        color = (0, 0, 255)  # colors(coloridx)   red
        c1, c2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
        cv2.rectangle(im, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)

    for box in fp_boxes:
        color = (255, 0, 0)  # colors(coloridx)   #blue
        c1, c2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
        cv2.rectangle(im, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
...
def drawtxt_to_image(imagepath, txtpath, gtFiles, dataset, method):
    height = 512.0
    width = 640.0

    readRGBdir = os.path.join(imagepath, "visible/test")
    readIRdir = os.path.join(imagepath, "infrared/test")
    ...
    p_gttxt = open(gtFiles, mode="r", encoding="utf-8")
    ...
    for fnRGB, fnIR in tqdm(zip(filenameRGB,filenameIR)):
        ...
        for line in lines:
            ...
            if conf > 0.25 and txt_image_id == imageid:
                prebbox.append([float(data[1]), float(data[2]), float(data[1])+float(data[3]), float(data[2])+float(data[4]), conf])  # bbox
            elif txt_image_id > imageid and prebbox:
                break

        gtbbox = []
        p_gttxt.seek(0)
        gtlines = p_gttxt.readlines()
        for line in gtlines:
            cleaned_line = line.strip()
            data = [ll for ll in cleaned_line.split(',')]
            txt_image_id = int(data[0])
            ...
            if txt_image_id == imageid:
                gtbbox.append([float(data[1]), float(data[2]), float(data[1])+float(data[3]), float(data[2])+float(data[4])])  # bbox
            elif txt_image_id > imageid and gtbbox:
                break

        # boxes = np.array([[x1, y1, x2, y2], ...])  # 示例框坐标
        # labels = np.array([label1, label2, ...])  # 示例类别标签
        # scores = np.array([score1, score2, ...])  # 示例置信度
        # true_labels = get_true_labels()

        to_remove = []
        tp_boxes = []
        fp_boxes = []
        fn_boxes = []  #gtbbox.copy()

        for det_box in prebbox:
            matched = False
            tmpiou = 0
            # if label in true_labels:
            for i, true_box in enumerate(gtbbox):
                # if det_box[5] == true_box[4]:  # 类别匹配
                iou = calculate_iou(det_box[:4], true_box[:4])
                if iou >= 0.5:  # 假设IoU阈值为0.5 and det_box[4] > 0.25
                    if matched is False:
                        tp_boxes.append(det_box[:4])   #模型正确地检测到了实际存在的目标。
                        to_remove.append(i)  # 标记需要移除的真实框索引
                        matched = True
                        tmpiou = iou
                    elif matched is True and iou > tmpiou:
                        tp_boxes[-1] = det_box[:4]  # 模型正确地检测到了实际存在的目标。
                        to_remove[-1] = i  # 标记需要移除的真实框索引
                        matched = True
                        tmpiou = iou
                    break
            if not matched:
                fp_boxes.append(det_box[:4])   #模型检测到了并不存在的目标。blue

        for i, true_box in enumerate(gtbbox):
            if i not in to_remove:
                fn_boxes.append(true_box[:4])   #实际存在目标，但模型没有检测到。red，错误检测

        # if prebbox:
        plot_one_box(tp_boxes, fn_boxes, fp_boxes, p_imageRGB, line_thickness=2)
        plot_one_box(tp_boxes, fn_boxes, fp_boxes, p_imageIR, line_thickness=2)
...
class KAISTPedEval(COCOeval):
    ...
    def draw_figure(ax, eval_results, methods, colors):
        ...
        plt.rcParams['font.size'] = 39  # 设置全局字体大小，可根据需要调整

        ax.spines['bottom'].set_linewidth(2.5)  # 加粗X轴
        ax.spines['left'].set_linewidth(2.5)  # 加粗Y轴
        ...
        for eval_result, method, color in zip(eval_results, methods, colors):
            ...
            ax.plot(xx[0], yy[0], color=color, linewidth=4.5, label=f'{mean_s:.1f}%, {method}')
        ...
        ax.set_ylim(0.01, 1)
        ax.set_xlim(2e-4, 50)
        ax.set_ylabel('Miss Rate', fontsize=60, fontfamily='Times New Roman', color='black')
        ax.set_xlabel('False Positives per Image (FPPI)', fontsize=60, fontfamily='Times New Roman', color='black')
...
def draw_allresult(multi_results, filename='figure.jpg'):
    ...
    filename: str
        Filename of figure
    """
    fig, axes = plt.subplots(3, 1, figsize=(25, 60))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rstFiles', type=str, nargs='+', default=['evaluation_script/KAIST_annotation.json'],
    ...
    gtFiles = './evaluation_script/state_of_arts/flir2/gt_flir_result.txt'

    # args.rstFiles = './evaluation_script/state_of_arts/flir/gt_flir_result.txt'
    # drawtxt_to_image("E:\\deeplearning\\dataset\\FLIR-align-3class\\", args.rstFiles, gtFiles, "flir", "Ground truth")

    # args.rstFiles = './evaluation_script/state_of_arts/flir2/RGB_result.txt'
    # YOLOv5_RGB_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # YOLOv5_RGB_results = sorted(YOLOv5_RGB_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # drawtxt_to_image("E:\\deeplearning\\dataset\\FLIR-align-3class\\", args.rstFiles, gtFiles, "flir", "RGB")
    #
    #
    # args.rstFiles = './evaluation_script/state_of_arts/flir2/T_result.txt'
    # YOLOv5_T_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # YOLOv5_T_results = sorted(YOLOv5_T_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\FLIR-align-3class\\", args.rstFiles, gtFiles, "flir", "T")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/flir2/RGB-T_result.txt'
    # YOLOv5_RGBT_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # YOLOv5_RGBT_results = sorted(YOLOv5_RGBT_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\FLIR-align-3class\\", args.rstFiles, gtFiles, "flir", "RGB-T")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/flir2/RGB-T+CAFE_result.txt'
    # YOLOv5_DCAFE_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # YOLOv5_DCAFE_results = sorted(YOLOv5_DCAFE_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\FLIR-align-3class\\", args.rstFiles, gtFiles, "flir", "RGB-T+CAFE")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/flir2/RGB-T+SAFE_result.txt'
    # YOLOv5_SAFE_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # YOLOv5_SAFE_results = sorted(YOLOv5_SAFE_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\FLIR-align-3class\\", args.rstFiles, gtFiles, "flir", "RGB-T+SAFE")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/flir2/RGB-T+DCSAFM_result.txt'
    # ours_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # ours_results = sorted(ours_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\FLIR-align-3class\\", args.rstFiles, gtFiles, "flir", "RGB-T+DCSAFM")
    # # #
    # compare_result = {
    #     'RGB': copy.deepcopy(YOLOv5_RGB_results),
    #     'T': copy.deepcopy(YOLOv5_T_results),
    #     'RGB-T': copy.deepcopy(YOLOv5_RGBT_results),
    #     'RGB-T+CAFE': copy.deepcopy(YOLOv5_DCAFE_results),
    #     'RGB-T+SAFE': copy.deepcopy(YOLOv5_SAFE_results),
    #     'RGB-T+DCSAFM': copy.deepcopy(ours_results),
    # }
    #
    # # Sort results by MR_all
    # draw_allresult(compare_result, filename=args.evalFig)

    # kaist
    # gtFiles = './evaluation_script/state_of_arts/kaist/gt_kaist_result.txt'
    # args.rstFiles = './evaluation_script/state_of_arts/kaist/gt_kaist_result.txt'
    # GT_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # GT_results = sorted(GT_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\kaist icaf\\", args.rstFiles, "kaist icaf", "Ground truth")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/kaist/AR-CNN_result.txt'
    # ARCNN_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # ARCNN_results = sorted(ARCNN_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\kaist icaf\\", args.rstFiles, gtFiles, "kaist icaf", "AR-CNN")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/kaist/CIANet_result.txt'
    # CIAN_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # CIAN_results = sorted(CIAN_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\kaist icaf\\", args.rstFiles, gtFiles, "kaist icaf", "CIANet")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/kaist/MBNet_result.txt'
    # MBNet_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # MBNet_results = sorted(MBNet_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\kaist icaf\\", args.rstFiles, gtFiles, "kaist icaf", "MBNet")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/kaist/MLPD_result.txt'
    # MLPD_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # MLPD_results = sorted(MLPD_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\kaist icaf\\", args.rstFiles, gtFiles, "kaist icaf", "MLPD")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/kaist/MSDS-RCNN_result.txt'
    # MSDS_RCNN_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # MSDS_RCNN_results = sorted(MSDS_RCNN_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\kaist icaf\\", args.rstFiles, gtFiles, "kaist icaf", "MSDS-RCNN")
    #
    # args.rstFiles = './evaluation_script/state_of_arts/kaist/Ours(DCSAFNet)_result.txt'
    # Ours_results = [evaluate(args.annFile, args.rstFiles, phase)]
    # Ours_results = sorted(Ours_results, key=lambda x: x['all'].summarize(0), reverse=True)
    # # drawtxt_to_image("E:\\deeplearning\\dataset\\kaist icaf\\", args.rstFiles, gtFiles, "kaist icaf", "Ours(DCSAFNet)")
    #
    # compare_result = {
    #     'AR-CNN': copy.deepcopy(ARCNN_results),
    #     'CIANet': copy.deepcopy(CIAN_results),
    #     'MBNet': copy.deepcopy(MBNet_results),
    #     # 'MLPD': copy.deepcopy(MLPD_results),
    #     'MSDS-RCNN': copy.deepcopy(MSDS_RCNN_results),
    #     'Ours': copy.deepcopy(Ours_results),
    # }
    #
    # # Sort results by MR_all
    # draw_allresult(compare_result, filename=args.evalFig)
    ...
    subdataset = "00" #"00"  "day_rgblost" "night_irlost"
    subdir =""  # "" # "\\corruption\\dual_modal\\"+subdataset
    gtFiles = ".\\state_of_arts\\"+dataset+"\\gt_"+dataset+"_"+subdataset+"_result.txt"
    #
    # args.rstFiles = ".\\state_of_arts\\"+dataset+"\\gt_"+dataset+"_"+subdataset+"_result.txt"
    # drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset, "Ground truth")
    #
    # args.rstFiles = ".\\state_of_arts\\"+dataset+"\\cdcyolofusion\\"+dataset+"_ir_rgb_45_"+subdataset+"\\cdcyolofusion_"+dataset+"_result.txt"
    # drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset,"cdcyolofusion")
    #
    # args.rstFiles = ".\\state_of_arts\\"+dataset+"\\cft\\"+dataset+"_ir_rgb_45_"+subdataset+"\\cft_"+dataset+"_result.txt"
    # drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset, "cft")
    #
    # args.rstFiles = ".\\state_of_arts\\"+dataset+"\\dacfusion\\"+dataset+"_ir_rgb_45_"+subdataset+"\\dacfusion_"+dataset+"_result.txt"
    # drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset,"dacfusion")
    #
    # args.rstFiles = ".\\state_of_arts\\"+dataset+"\\deyolo\\"+dataset+"_ir_rgb_45_"+subdataset+"\\deyolo_"+dataset+"_result.txt"
    # drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset, "deyolo")
    #
    # args.rstFiles = ".\\state_of_arts\\"+dataset+"\\icaf\\"+dataset+"_ir_rgb_45_"+subdataset+"\\icaf_"+dataset+"_result.txt"
    # drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset,"icaf")
    #
    # args.rstFiles = ".\\state_of_arts\\"+dataset+"\\mstf\\"+dataset+"_ir_rgb_45_"+subdataset+"\\mstf_"+dataset+"_result.txt"
    # drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset,"mstf")
    #
    args.rstFiles = ".\\state_of_arts\\"+dataset+"\\ours\\"+dataset+"_ir_rgb_45_"+subdataset+"\\ours_"+dataset+"_result.txt"
    drawtxt_to_image("E:\\deeplearning\\dataset\\"+dataset+subdir, args.rstFiles, gtFiles, dataset+"_"+subdataset,"ours")

    subdataset = "day_rgblost"  # "00"  "day_rgblost" "night_irlost"
    subdir = "\\corruption\\dual_modal\\"+subdataset
    gtFiles = ".\\state_of_arts\\" + dataset + "\\gt_" + dataset + "_" + subdataset + "_result.txt"
    #
    # args.rstFiles = ".\\state_of_arts\\" + dataset + "\\gt_" + dataset + "_" + subdataset + "_result.txt"
    ...
    subdataset = "night_irlost"  # "00"  "day_rgblost" "night_irlost"
    subdir = "\\corruption\\dual_modal\\" + subdataset
    gtFiles = ".\\state_of_arts\\" + dataset + "\\gt_" + dataset + "_" + subdataset + "_result.txt"
