...
class CrossSpatialAttention(nn.Module):
    def __init__(self, d_model, d_k, d_v, h = 8, attn_pdrop=.1, resid_pdrop=.1):
    ...
    def forward(self, query, key, value):
        fea_flat = query
        b_s, nq = fea_flat.shape[:2]
        nk = fea_flat.shape[1]

        q_fea = self.LN1(query)
        k_fea = self.LN2(key)
        v_fea = self.LN3(value)

        q = self.que_proj(q_fea).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, nq, d_k)
        k = self.key_proj(k_fea).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, d_k, nk) K^T
        v = self.val_proj(v_fea).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)
...
class CrossChannelAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        k_fea = self.LN2(key)
        v_fea = self.LN3(value)
        q = self.que_proj(q_fea).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, nq, d_k)
        k = self.key_proj(k_fea).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, d_k, nk) K^T
        v = self.val_proj(v_fea).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 3, 1)  # (b_s, h, nk, d_v)
        ...
        # output
        out = torch.matmul(att, v).permute(0, 3, 1, 2).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out = self.resid_drop(self.out_proj(out))  # (b_s, nq, d_model)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        fea_flat = self.LN(fea_flat)
        q = self.que_proj(fea_flat).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, nq, d_k)
        k = self.key_proj(fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, d_k, nk) K^T
        v = self.val_proj(fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        fea_flat = self.LN(fea_flat)
        q = self.que_proj(fea_flat).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, nq, d_k)
        k = self.key_proj(fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, d_k, nk) K^T
        v = self.val_proj(fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 3, 1)  # (b_s, h, nk, d_v)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        q_vis = self.que_proj_vis(rgb_fea_flat).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, dk, nq)
        k_vis = self.key_proj_vis(rgb_fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, nk, d_k) K^T
        v_vis = self.val_proj_vis(rgb_fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 3, 1)  # (b_s, h, d_v, nk)
        ...
        q_ir = self.que_proj_ir(ir_fea_flat).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 3, 1)  # # (b_s, h, dk, nq)
        k_ir = self.key_proj_ir(ir_fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, nk, d_k) K^T
        v_ir = self.val_proj_ir(ir_fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 3, 1)  # (b_s, h, d_v, nk)
        ...
        out_vis = torch.matmul(att_vis, v_vis).permute(0, 3, 1, 2).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_vis = self.resid_drop(self.out_proj_vis(out_vis)) # (b_s, nq, d_model)
        out_ir = torch.matmul(att_ir, v_ir).permute(0, 3, 1, 2).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_ir = self.resid_drop(self.out_proj_ir(out_ir)) # (b_s, nq, d_model)

        return [out_vis, out_ir]


class SelfTransformer1(nn.Module):
    def __init__(self, d_model, d_k, d_v, h = 8, block_exp = 4, attn_pdrop = .1, resid_pdrop = .1):
        super(SelfTransformer1, self).__init__()
        self.ch_att = ChannelAttention(d_model, d_k, d_v, h, attn_pdrop, resid_pdrop)
        self.sp_att = SpatialAttention(d_model, d_k, d_v, h, attn_pdrop, resid_pdrop)
        ...
        self.coefficient0 = LearnableCoefficient()
    ...
    def forward(self, x):
        # ch_fea_out = self.ch_att(x)
        # sp_fea_out = self.sp_att(ch_fea_out)
        # ch_fea_out = self.ch_att(x)
        # sp_fea_out = self.sp_att(x)
        sp_fea_out = self.sp_att(x)
        ch_fea_out = self.ch_att(sp_fea_out)
        new_fea = self.coefficient0(x) + self.coefficient1(sp_fea_out) + self.coefficient2(ch_fea_out)
        new_out = self.coefficient3(new_fea) + self.coefficient4(self.mlp(self.LN(new_fea)))

        return new_out


class RGBSelfFeatureEnhancement(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h = 8, block_exp = 4, attn_pdrop = .1, resid_pdrop = .1):
        super(RGBSelfFeatureEnhancement, self).__init__()
        ...
        self.selfatt = SelfTransformer1(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop)
    ...
    def forward(self, x):
        bs, c, h, w = x.shape
        new_fea = self.coefficient(self.avgpool(x), self.maxpool(x))
        new_c, new_h, new_w = new_fea.shape[1], new_fea.shape[2], new_fea.shape[3]
        fea_in = new_fea.contiguous().view(bs, new_c, -1).permute(0, 2, 1) + self.pos_emb

        new_fea = self.selfatt(fea_in)

        new_fea = new_fea.contiguous().view(bs, new_h, new_w, new_c).permute(0, 3, 1, 2)
        if self.training == True:
            new_fea = F.interpolate(new_fea, size=([h, w]), mode='nearest')
        else:
            new_fea = F.interpolate(new_fea, size=([h, w]), mode='bilinear')
        new_out = new_fea + x
...
class SelfTransformer2(nn.Module):
    def __init__(self, d_model, d_k, d_v, h = 8, block_exp = 4, attn_pdrop = .1, resid_pdrop = .1):
        super(SelfTransformer2, self).__init__()
        self.ch_att = ChannelAttention(d_model, d_k, d_v, h, attn_pdrop, resid_pdrop)
        self.sp_att = SpatialAttention(d_model, d_k, d_v, h, attn_pdrop, resid_pdrop)
        self.mlp = nn.Sequential(nn.Linear(d_model, block_exp * d_model),
                                 nn.GELU(),
...
class IRSelfFeatureEnhancement(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h = 8, block_exp = 4, attn_pdrop = .1, resid_pdrop = .1):
        super(IRSelfFeatureEnhancement, self).__init__()
        ...
        # init weights
        self.apply(self._init_weights)
        self.selfatt = SelfTransformer2(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop)
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        rgb_fea_flat = self.coefficient5(rgb_att_out) + self.coefficient6(self.mlp_vis(self.LN1(rgb_att_out)))
...
class CrossModalityFeatureEnhancement(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h=8, block_exp=4, attn_pdrop=0.1, resid_pdrop=0.1):
        super(CrossModalityFeatureEnhancement, self).__init__()
        ...
        # cross transformer
        self.crosstransformer = CrossTransformer(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop)
    ...
    def forward(self, x):
        ...
        #2.直通。此处将源程序改为直通
        # new_fea = self.concat([rgb_fea, ir_fea])
...
class DCSAFeatureEnhancement(nn.Module):
    def __init__(self, d_model,vert_anchors=16, horz_anchors=16, h=8, block_exp=4, attn_pdrop=0.1, resid_pdrop=0.1):
        super(DCSAFeatureEnhancement, self).__init__()
        self.n_embd = d_model
        self.vert_anchors = vert_anchors
        self.horz_anchors = horz_anchors
        d_k = d_model
        ...
        # positional embedding parameter (learnable), rgb_fea + ir_fea
        self.pos_emb_vis = nn.Parameter(torch.zeros(1, self.vert_anchors * self.horz_anchors, self.n_embd))
        self.pos_emb_ir = nn.Parameter(torch.zeros(1, self.vert_anchors * self.horz_anchors, self.n_embd))
        self.pos_emb_dwave = nn.Parameter(torch.zeros(1, self.vert_anchors * self.horz_anchors, self.n_embd))

        self.avgpool = AdaptivePool2d(self.vert_anchors, self.horz_anchors, 'avg')
        self.maxpool = AdaptivePool2d(self.vert_anchors, self.horz_anchors, 'max')
        self.concat = Concat(dimension=1)
        self.conv1x1_2 = Conv(c1=d_model * 2, c2=d_model, k=1, s=1, p=0, g=1, act=True)
        ...
        # cross transformer
        self.crosstransformer = CrossTransformer(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop)
        self.Sharedtransformer = SelfTransformer(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop)
    ...
    def forward(self, x):
        rgb_fea = x[0]
        ir_fea = x[1]
        dwave_fea = self.conv1x1_2(self.concat([rgb_fea, ir_fea]))
        ...
        dwave_fea = self.conv1x1_2(self.concat([new_rgb_fea, new_ir_fea]))
        new_dwave_fea = self.dwave_coefficient(self.avgpool(dwave_fea), self.maxpool(dwave_fea))
        dwave_fea_flat_in = new_dwave_fea.contiguous().view(bs, new_c, -1).permute(0, 2, 1) + self.pos_emb_dwave
        dwave_fea_flat = self.Sharedtransformer(dwave_fea_flat_in)
        ...
        new_dwave_fea = dwave_fea_CFE + dwave_fea

        new_out = self.conv1x1_3(self.concat([new_rgb_fea, new_ir_fea, new_dwave_fea]))
...
class VisBranch(nn.Module):
    def __init__(self, in_channnel, vert_anchors=16, horz_anchors=16, h=8):
        super(VisBranch, self).__init__()
        self.vert_anchors = vert_anchors
        self.horz_anchors = horz_anchors
        self.h = h
        self.attention = RGBSelfFeatureEnhancement(in_channnel, self.vert_anchors, self.horz_anchors, self.h)
        # self.FMModule = FeatureMappingModule(in_channnel, in_channnel, num_experts=32, top_k=2)

    def forward(self, x):
        vis_fea = x[0]
        out = self.attention(vis_fea)
        return out
...
class IRBranch(nn.Module):
    def __init__(self, in_channnel, vert_anchors=16, horz_anchors=16, h=8):
        ...
        self.horz_anchors = horz_anchors
        self.h = h
        self.attention = IRSelfFeatureEnhancement(in_channnel, self.vert_anchors, self.horz_anchors, self.h)
        # self.FMModule = FeatureMappingModule(in_channnel, in_channnel, num_experts=32, top_k=2)

    def forward(self, x):
        ir_fea = x[1]
        out = self.attention(ir_fea)
        return out
...
class FusionBranch(nn.Module):
    def __init__(self, in_channnel, vert_anchors=16, horz_anchors=16, h=8):
        ...
        self.horz_anchors = horz_anchors
        self.h = h
        self.fusion = CrossModalityFeatureEnhancement(in_channnel, self.vert_anchors, self.horz_anchors, self.h)
        # self.concat = Concat(dimension=1)
        # self.conv1x1_2 = Conv(c1=in_channnel * 2, c2=in_channnel, k=1, s=1, p=0, g=1, act=True)
        # self.FMModule = FeatureMappingModule(in_channnel, in_channnel, num_experts=32, top_k=2)
    ...
    def forward(self, x):
        ...
        # ir_fea = x[1]
        out = self.fusion(x)
        # dwave_fea = self.concat([vis_fea, ir_fea])
        # out = self.conv1x1_2(dwave_fea)
        # out = self.FMModule(out)

        return out


class gateAtteention(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h=8, num_modal=3, block_exp=4, attn_pdrop=0.1, resid_pdrop=0.1):
        super().__init__()
        d_k = d_model
        d_v = d_model
        n_embd = d_model
        ...
        self.pos_emb_com = nn.Parameter(torch.zeros(1, self.vert_anchors * self.horz_anchors, n_embd))
        self.pos_emb_dif = nn.Parameter(torch.zeros(1, self.vert_anchors * self.horz_anchors, n_embd))
        ...
        self.crosstransformer = CrossSpatialAttention(d_model, d_k, d_v, h, attn_pdrop, resid_pdrop)
        self.gateout = nn.Linear(d_model, self.num_modal)
    ...
    def forward(self, x):
        vis_fea = x[0]
        ir_fea = x[1]
        assert vis_fea.shape[0] == ir_fea.shape[0]
        bs, c, h, w = vis_fea.shape

        new_vis_fea = self.vis_coefficient(self.avgpool(vis_fea), self.maxpool(vis_fea))
        new_ir_fea  = self.ir_coefficient(self.avgpool(ir_fea), self.maxpool(ir_fea))

        com_fea = new_vis_fea+new_ir_fea
        dif_fea = new_vis_fea-new_ir_fea

        new_c, new_h, new_w = com_fea.shape[1], com_fea.shape[2], com_fea.shape[3]
        com_fea_flat_in = new_vis_fea.contiguous().view(bs, new_c, -1).permute(0, 2, 1) + self.pos_emb_com

        dif_fea_flat_in = dif_fea.contiguous().view(bs, new_c, -1).permute(0, 2, 1) + self.pos_emb_dif

        fea_flat = self.crosstransformer(dif_fea_flat_in, com_fea_flat_in, com_fea_flat_in)
        fea_CFE = fea_flat.contiguous().view(bs, new_h, new_w, new_c).permute(0, 3, 1, 2)

        fea_pooled = F.adaptive_avg_pool2d(fea_CFE, (1, 1)).view(bs, c)  # 对通道维度进行全局平均池化，得到 (bs, c)
        out = self.gateout(fea_pooled)

        return out


class DFE(nn.Module):
    def __init__(self, input_dim, vert_anchors=16, horz_anchors=16, h=8, num_modal=3, top_k=1):
        ...
        self.gate = nn.Linear(self.in_channels*2, self.num_modal)
        # self.gate = gateAtteention(self.in_channels, vert_anchors=self.vert_anchors, horz_anchors=self.horz_anchors, h=1, num_modal=self.num_modal)
        self.experts1 = VisBranch(self.in_channels, self.vert_anchors, self.horz_anchors, self.h)   # RGB分支，当选中此路时，进行RGB特征映射
        self.experts2 = FusionBranch(self.in_channels, self.vert_anchors, self.horz_anchors, self.h) #nn.Sequential(DCSAModule(self.in_channels), MappingModule(self.in_channels, self.out_channels))  # 融合分支，当选中此路时，进行RGB和IR特征融合，然后进行映射
        self.experts3 = IRBranch(self.in_channels, self.vert_anchors, self.horz_anchors, self.h)  # Thermal分支，当选中此路时，进行Thermal特征映射

    def forward(self, x):
        bs, c, h, w = x[0].shape

        # gate_scores = self.gate(x)  # 训练最终网络时，采用评分机制选择路径

        x_vis_pooled = F.adaptive_avg_pool2d(x[0], (1, 1)).view(bs, c)  # 对通道维度进行全局平均池化，得到 (bs, c)
        x_ir_pooled = F.adaptive_avg_pool2d(x[1], (1, 1)).view(bs, c)  # 对通道维度进行全局平均池化，得到 (bs, c)
        x_comb_pooled = torch.cat([x_vis_pooled, x_ir_pooled], dim=1)
        gate_scores = self.gate(x_comb_pooled)                    #训练最终网络时，采用评分机制选择路径
...
class FusionModule(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h=8):
        super(FusionModule, self).__init__()
        self.vert_anchors = vert_anchors
        self.horz_anchors = horz_anchors
        self.h = h
        self.d_model = d_model
        self.dfeModule = DFE(self.d_model, self.vert_anchors, self.horz_anchors, self.h)

        # self.concat = Concat(dimension=1)
        # self.conv1x1_2 = Conv(c1=d_model * 2, c2=d_model, k=1, s=1, p=0, g=1, act=True)
        # self.FMModule = FeatureMappingModule(d_model, d_model, num_experts=64, top_k=2)
    ...
    def forward(self, x):
        ...
        dwave_fea = [vis_fea, ir_fea]
        new_out, quality_pred = self.dfeModule(dwave_fea)
        return new_out, quality_pred

        # new_out = self.FAModule(dwave_fea)
        # dwave_fea = self.concat([vis_fea, ir_fea])
        # out = self.conv1x1_2(dwave_fea)
        # new_out = self.FMModule(new_out)
        # new_out = new_out
