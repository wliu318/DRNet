...
class FaultAwareLoss(nn.Module):
    ...
    def forward(self, preds, target):
        ...
        if preds.shape != target.shape:
            raise ValueError(f"预测值batch size {preds.shape}与标签batch size {target.shape}不匹配")
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.fault_aware_loss = FaultAwareLoss()

    def __call__(self, p, quality_pred, targets, quality_label):  # predictions, targets, model
        device = targets.device

        # if quality_pred is not None and quality_label is not None:
        #     assert quality_pred.shape == quality_label.shape
        #     lquality = self.fault_aware_loss(quality_pred, quality_label) * self.hyp['quality']
        #     lquality = lquality.reshape(1).to(device)
        # else:
        #     warnings.warn(f"no Quality labels or quality_pred")
        #     lquality = torch.zeros(1, device=device)

        if quality_pred is not None and quality_label is not None:
            # 检查是否有空张量
            if quality_pred.numel() == 0 or quality_label.numel() == 0:
                print("Warning: Empty quality tensor detected, skipping quality loss")
                lquality = torch.zeros(1, device=device)
            elif quality_pred.shape != quality_label.shape:
                # 记录详细信息以便调试
                print(f"Shape mismatch - quality_pred: {quality_pred.shape}, quality_label: {quality_label.shape}")
                print(f"quality_pred: {quality_pred}")
                print(f"quality_label: {quality_label}")

                # 使用最小的形状
                min_shape = min(quality_pred.shape[0], quality_label.shape[0])
                if len(quality_pred.shape) > 1 and len(quality_label.shape) > 1:
                    min_features = min(quality_pred.shape[1], quality_label.shape[1])
                    quality_pred = quality_pred[:min_shape, :min_features]
                    quality_label = quality_label[:min_shape, :min_features]
                else:
                    quality_pred = quality_pred[:min_shape]
                    quality_label = quality_label[:min_shape]
        ...
        loss = lbox + lobj + lcls + lrk + lquality
        return loss * bs, torch.cat((lbox, lobj, lcls, lrk, lquality)).detach()
