...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        self.path = path
        self.quality_label_dir = os.path.join(path, 'qualities')
    ...
    #         # 构建质量标签路径：将`images`替换为`qualities`，后缀改为.txt
    #         img_path = Path(img_path)
    #         parts = list(img_path.parts)
    #         try:
    #             split_idx = next(i for i, part in enumerate(parts) if part.lower() in ('train', 'val', 'test'))
    #             if split_idx > 0:
    #                 parts[split_idx - 1] = 'qualities'
    #             quality_path = Path(*parts).with_suffix('.txt')
    #
    #             try:
    #                 with open(quality_path, 'r') as f:
    #                     q = list(map(float, f.read().strip().split()))
    #                     qualities.append(torch.tensor(q, dtype=torch.float32))
    #             except FileNotFoundError:
    #                 default_q = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32)
    #                 qualities.append(default_q)  # 默认值（可选警告日志）
    #                 print(f"警告：缺失质量标签 {quality_path}")
    #         except StopIteration:
    #             default_q = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32)
    #             qualities.append(default_q)
    #             raise ValueError("Couldn't find 'train'/'val' in path")
    #
    #     qualities = [q.unsqueeze(0) if q.dim() == 1 else q for q in qualities]
    #
    #     # # 3. 确定质量标签的维度
    #     # num_quality_values = len(qualities[0])  # 假设所有质量标签长度相同
    #     # # 原始YOLO标签有6列：[img_idx, class, x, y, w, h]
    #     # # 现在扩展为 6 + num_quality_values 列
    #     # targets = torch.zeros((sum(len(l) for l in label), 6 + num_quality_values))
    #     #
    #     # # 4. 填充数据
    #     # row_offset = 0
    #     # for i, (l, q) in enumerate(zip(label, qualities)):
    #     #     num_objects = len(l)
    #     #     # 填充原始标签（前6列）
    #     #     targets[row_offset:row_offset + num_objects, :6] = l
    #     #     # 填充质量标签（后num_quality_values列）
    #     #     for q_idx in range(num_quality_values):
    #     #         targets[row_offset:row_offset + num_objects, 6 + q_idx] = q[q_idx]
    #     #     row_offset += num_objects
    #
    #     return torch.stack(img, 0), torch.cat(label, 0), path, shapes, torch.cat(qualities, 0)
    def collate_fn(batch):
            ...
            try:
                ...
                try:
                    with open(quality_path, 'r') as f:
                        ...
                        qualities.append(q_tensor.unsqueeze(0))  # 确保至少有2维
                except FileNotFoundError:
                    # 创建与其他质量标签相同形状的默认张量
                    default_q = torch.zeros(1, 3, dtype=torch.float32)  # 假设质量标签是3维的
                    qualities.append(default_q)
                    print(f"警告：缺失质量标签 {quality_path}")
            except StopIteration:
                default_q = torch.zeros(1, 3, dtype=torch.float32)  # 假设质量标签是3维的
                qualities.append(default_q)
                print(f"警告：路径中未找到'train'/'val'目录: {img_path}")

        # 确保所有张量形状一致
        try:
            qualities_tensor = torch.cat(qualities, 0)
        except RuntimeError as e:
            print(f"连接质量张量时出错: {e}")
            print("各质量张量形状:", [q.shape for q in qualities])
            # 使用第一个非空张量的形状作为参考
            ref_shape = next(q.shape for q in qualities if q.numel() > 0)
            padded_qualities = []
            for q in qualities:
                if q.shape != ref_shape:
                    padded_q = torch.zeros(ref_shape, dtype=torch.float32)
                    padded_qualities.append(padded_q)
                else:
                    padded_qualities.append(q)
            qualities_tensor = torch.cat(padded_qualities, 0)

        return torch.stack(img, 0), torch.cat(label, 0), path, shapes, qualities_tensor
...
def load_image_rgb_ir(self, index):
    ...
    #     return self.imgs_rgb[index], self.imgs_ir[index], self.img_hw0_rgb[index], self.img_hw_rgb[index]  # img, hw_original, hw_resized
        h0_rgb, w0_rgb = img_rgb.shape[:2]
        h0_ir, w0_ir = img_ir.shape[:2]

        # 使用相同的缩放比例
        r_rgb = self.img_size / max(h0_rgb, w0_rgb)
        r_ir = self.img_size / max(h0_ir, w0_ir)

        # 确保使用相同的目标尺寸
        target_size = (int(w0_rgb * r_rgb), int(h0_rgb * r_rgb))

        if r_rgb != 1:
            img_rgb = cv2.resize(img_rgb, target_size,
                                 interpolation=cv2.INTER_AREA if r_rgb < 1 and not self.augment else cv2.INTER_LINEAR)

        # 确保红外图像与RGB图像尺寸完全一致
        if img_ir.shape[:2] != img_rgb.shape[:2]:
            img_ir = cv2.resize(img_ir, (img_rgb.shape[1], img_rgb.shape[0]),
                                interpolation=cv2.INTER_AREA if r_ir < 1 and not self.augment else cv2.INTER_LINEAR)

        return img_rgb, img_ir, (h0_rgb, w0_rgb), img_rgb.shape[:2]
    else:
