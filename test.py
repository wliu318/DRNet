...
def test(data,
    ...
    if nc == 1:
        s = ('%20s' + '%12s' * 11) % ('Class', 'Images', 'Labels', 'TP', 'FP', 'FN', 'F1', 'P', 'R', 'mr-2', 'mAP@.5', 'mAP@.5:.95')  # 设置进度条的显示信息
    else:
    ...
    for batch_i, (img, targets, paths, shapes, quality) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, _, train_out, quality_pred = model(img_rgb, img_ir, augment=augment)  # inference and training outputs
            t0 += time_synchronized() - t

            # Compute loss
            if compute_loss:
                loss += compute_loss([x.float() for x in train_out], quality_pred, targets, quality)[1][:4]  # box, obj, cls
    ...
    if nc > 1:
        ...
        logger.info(pf % ('all', seen, nt.sum(), mp, mr, np.mean(mr_2), map50, map75, map))
    else:
        pf = '%20s' + '%12i' * 2 + '%12.4g' * 9  # print format
        logger.info(pf % ('all', seen, nt.sum(), tp, fp, fn, f1, mp, mr, mr_2, map50, map))
    ...
    # Print speeds
    t = tuple(x / seen * 1E3 for x in (t0, t1, t0 + t1)) + (imgsz, imgsz, batch_size)  # tuple,t0:推理时间，t1:nms时间
    if not training:
    ...
    if not isinstance(tp, int):
        #return (tp[0], fp[0], fn[0], f1[0], mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist(), mr_2.mean()), maps, MRresult, t
        return (tp.mean(), fp.mean(), fn.mean(), f1.mean(), mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist(), mr_2.mean()), maps, MRresult, t
    else:
        return (tp, fp, fn, f1, mp, mr, map50, map,*(loss.cpu() / len(dataloader)).tolist(), mr_2), maps, MRresult, t
...
#    parser.add_argument('--weights', nargs='+', type=str, default='/home/shen/Chenyf/exp_save/multispectral-object-detection/5l_FLIR_3class_transformerx2_avgpool+maxpool/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default='./runs/kaist_ir_rgb_45_rgbca(sa)_irconcat_moe/weights/best.pt', help='model.pt path(s)')
    ...
    p = "e:/deeplearning/dataset/kaist//corruption/dual_modal/night_irlost/labels/test"
